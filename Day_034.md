...
## Solution
1. SSH to Storage server, cd into the `/usr/src/kodekloudrepos/blog` and checkout to master branch
```sh
...
3. Copy the post-update.sample to other file and give executable permissions
```sh
...
4. Add the below script into the file by using command `vi /opt/blog.git/hooks/post-update`
```sh
#!/bin/sh
DATE=$(date +%F)
...
6. Fetch the tags and verify the released tag
```sh
git fetch --tags
git tag
