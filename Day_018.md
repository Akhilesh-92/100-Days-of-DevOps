...
## Solution
1. Install `httpd` and `PHP` on all App Servers and Execute the following commands on all app servers.
```sh
...
3. Create database, user and grant all privileges using `mysql` interactive shell
```sh
sudo mysql
