...
## Solution
1. *Login to Jenkins server remotely*
    ```
...
2. *Go to Manage Jenkins -> Available Plugins -> Publish over SSH -> Install -> restart Jenkins*
  
3. *Go to Manage Jenkins -> System -> Publish over SSH -> Add SSH Servers -> Database server Name, hostname, username, password*

4. *Login to Jenkins*
    * Create a Freestyle job
    * Build periodically -> */10 * * * *
    * Build with steps -> Send files or execute commands over ssh -> Add below commands
        ```
...
6. *Verify copied `.sql` file in backup server with time stamp*
    ```
    ls -lr db_backups/
    ```
