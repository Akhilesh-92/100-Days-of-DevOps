...
*Go to Manage Jenkins -> Available Plugins -> Publish over SSH -> Install -> restart Jenkins*
  
*Go to Manage Jenkins -> System -> Publish over SSH -> Add SSH Servers -> Database server Name, hostname, username, password*
...
*Verify copied `.sql` file in backup server with time stamp*
```
ls -lr db_backups/
```
