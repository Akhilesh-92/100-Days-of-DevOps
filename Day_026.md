...
## Solution
SSH to Storage server, check the remote and verify we are in `master` branch
```sh
...
Add new remote `dev_apps` and verify the remote creation
```sh
git remote add dev_apps /opt/xfusioncorp_apps.git
git remote -v
```

Copy the file `/tmp/index.html` to the current folder, add and commit the file
```sh
cp /tmp/index.html .
git add .
...
Finally push the committed changes to remote origin created
```sh
