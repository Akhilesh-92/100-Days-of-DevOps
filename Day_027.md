...
## Solution
SSH to Storage server and cd into the repo `/usr/src/kodekloudrepos/demo`
```sh
cd /usr/src/kodekloudrepos/demo
```

Verify git log to find initial commit message
```sh
sudo git log --oneline
```

Now, revert to the inital commit message
```sh
