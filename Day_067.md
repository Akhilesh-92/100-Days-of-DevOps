...
## Solution
On jump host, create `redis-master` deployment using below details
```sh
apiVersion: apps/v1
kind: Deployment
metadata:
...
Create a service `redis-master` 
```sh
kubectl expose deployment redis-master --port 6379 --type ClusterIp --name=redis-master
```

Create `redis-slave` deployment using yaml details below
```sh
apiVersion: apps/v1
kind: Deployment
metadata:
...
Create a service `redis-slave` 
```sh
kubectl expose deployment redis-slave --port 6379 --type ClusterIp --name=redis-slave
```

Create `frontend` deployment using yaml file details below
```sh
apiVersion: apps/v1
kind: Deployment
metadata:
...
Verfify the deployemnts and pods are in running state
```sh
kubectl get deploy
kubectl get pods
