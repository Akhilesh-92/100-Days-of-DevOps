...
## Solution
1. SSH to Storage server and cd into the repo
```sh
...
3. Remove the commits to point back HEAD to the commit message `add data.txt file`
```sh
git reset --hard HEAD~10
git push origin -f master
```

4. Verify there are only two commits in the commit history i.e initial commit and add data.txt file
```sh
git log --oneline
```
