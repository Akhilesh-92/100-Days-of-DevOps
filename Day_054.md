...
## Solution
1. On the jump host, create a pod.yaml file
```sh
kubectl run volume-share-nautilus --image fedora:latest --dry-run=client -o yaml > pod.yaml
```


2. Modify the pod.yaml file in the containers secton like below
```
  containers:
  - image: fedora:latest
...
3. Create the pod using updated pod.yaml
```sh
kubectl create -f pod.yaml
```


4. Exec into the first container and create a file `blog.txt` inside `/bin/bash` shell
```sh
...
5. Verify the second container has `blog.txt` file under `/tmp/games` path
```sh
