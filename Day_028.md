...
## Solution
SSH into the Storage server and cd into the repo
```sh
cd /usr/src/kodekloudrepos
git checkout master
```

Note the commit ID of the commit by using the message `Update info.txt`
```sh
git log feature --oneline
```

Cherry pick the changes of commit into the master branch and push the changes
```sh
