...
## Solution
1. *Go to Manage Jenkins -> Available Plugins -> Git, Publish Over SSH -> Install -> restart Jenkins*

2. *Go to Manage Jenkins -> System -> SSH Servers -> Add SSH Servers -> Add all 3 app servers with Name, hostname, username, password*

3. *Create primary deployment Job*

     * Job Name: `nautilus-app-deployment`
     * Type: Freestyle Project
     * SCM: GIT -> Add Repository URL
     * Build Steps -> Shell Execute
     ```
...
5. *Goto nautilus-app-deployment job and add post build action -> Select `manage-services` project -> Trigger only if build is stable*

6. *Build nautilus-app-deployment job which will trigger `manage-services` job*

7. *Verify build is successfull and click on APP button*
