...
## Solution
On jump host, create a namespace
```sh
...
Create two deployemnts dep1.yaml and dep2.yaml with contents given below
```sh
apiVersion: apps/v1
kind: Deployment
metadata:
...
Create two deployments with above yaml file contents
```sh
...
Create a ClusterIP service and NodePort service
```sh
