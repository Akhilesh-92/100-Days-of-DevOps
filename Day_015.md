...
## Solution
Install `nginx` service on `App Server 3`
```sh
...
Copy `.crt` and `.key` contents to `/etc/pki/nginx/server.crt` and `/etc/pki/nginx/private/server.crt` respectively
```sh
sudo cp /tmp/nautilus.crt /etc/pki/nginx/server.crt
sudo cp /tmp/nautilus.key /etc/pki/nginx/private/server.crt
```

Create index.html file and write the content `Welcome!`
```sh
