...
## Solution
Install nginx and start it on `app server 1`
```sh
...
Install `php-fpm` and `cat /etc/redhat-release`, if output is 9, then proceed with below commands
```sh
...
Once php-fpm is installed, start and verify the installation
```sh
...
Next update the unit sock file and required fields in php-fpm config file as like below
> sudo vi /etc/php-fpm.d/www.conf --> listen = /var/run/php-fpm/default.sock --> user = nginx --> group = nginx


Restart php-fpm, create parent dir and enable it
```sh
...
Edit the nginx.conf file and update the following fields.
```sh
...
Verify the same using curl command
```sh
