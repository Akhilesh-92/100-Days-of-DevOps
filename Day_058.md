...
## Solution
On jump host, create deployment resource
```sh
k create deployment grafana-deployment-nautilus --image grafana/grafana
```


Update grafana port in deployement
```sh
k edit deployment grafana-deployment-nautilus
```


Add container port in deployemnt like below
```sh 
ports:
...
Verify Grafana login page access
```sh
