...
## Solution
SSH to App server 1 and cd into the `/python_app` directory
```sh
ssh tony@Ir0nM@n
cd /python_app
```
...
Create a Dockerfile here in `/python_app` directory and dump the following
```
FROM python:alpine
COPY src/ .
...
Build an image and create a contaner
```sh
