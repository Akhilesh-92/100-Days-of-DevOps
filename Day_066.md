...
## Solution
On jump host, create pv.yamland pvc.yaml files with below details
```sh
apiVersion: v1
...
Create pv and pvc using yaml files and verify the status to be `Bound`
```sh
kubectl create -f pv.yaml
kubectl create -f pvc.yaml
```

Create dep.yaml file with details below
```sh
apiVersion: apps/v1
kind: Deployment
metadata:
